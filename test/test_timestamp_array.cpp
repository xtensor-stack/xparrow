...
namespace sparrow
{
    ...
    const std::chrono::time_zone* new_york = std::chrono::locate_zone("America/New_York");
            ...
            SUBCASE("insert")
            {
                SUBCASE("with pos and value")
                {
                    ...
                    SUBCASE("in the middle")
                    {
                        timestamp_array<T> ar(new_york, input_values);
                        const auto new_value = make_nullable<T>(make_value<T>(99));
                        auto pos = ar.cbegin() + input_values.size() / 2;
                        ar.insert(pos, new_value);
                        for (size_t i = 0; i < input_values.size() / 2; ++i)
                        {
                            CHECK_EQ(ar[i], input_values[i]);
                        }
                        CHECK_EQ(ar[input_values.size() / 2], new_value);
                        for (size_t i = input_values.size() / 2; i < ar.size() - 1; ++i)
                        {
                            CHECK_EQ(ar[i + 1], input_values[i]);
                        }
                    }
                ...
                SUBCASE("with pos, count and value")
                {
                    ...
                    SUBCASE("in the middle")
                    {
                        ...
                        CHECK_EQ(ar[input_values.size() / 2], new_value);
                        CHECK_EQ(ar[input_values.size() / 2 + 1], new_value);
                        for (size_t i = input_values.size() / 2; i < ar.size() - 2; ++i)
                        {
                            CHECK_EQ(ar[i + 2], input_values[i]);
                        }
                    }
            ...
            SUBCASE("erase")
            {
                SUBCASE("with pos")
                {
                    ...
                    SUBCASE("in the middle")
                    {
                        ...
                        for (size_t i = input_values.size() / 2; i < ar.size(); ++i)
                        {
                            CHECK_EQ(ar[i], input_values[i + 1]);
                        }
                    }
